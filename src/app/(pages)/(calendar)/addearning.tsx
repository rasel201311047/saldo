...
import { usePostIncomeDataMutation } from "@/src/redux/api/Page/calendar/calendarApi";
...
const Addearning = () => {
  ...
  const [dataEarngPost, { isLoading: earninbgload }] =
  ...
  const checkboxToBoolean = (value) => {
    return value === "yes";
  };
  const hamdleSaveIncome = async () => {
    const playload = {
      category: category?.title,
      amount: ammount,
      accountId: accountNumber,
      fillForAllYear: checkboxToBoolean(fillCheckbox),
      date: new Date(date).toISOString(),
      time: formatTimeToHHMM(time),
      name: name,
    };
    console.log("hello earnig data", playload);

    try {
      console.log("Sending payload:", playload);
      const res = await dataEarngPost(playload).unwrap();
